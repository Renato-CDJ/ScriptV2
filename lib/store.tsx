...
export function initializeMockData() {
  ...
  // Load users from Firebase first, then initialize mock data if Firebase is empty
  getAllUsersAsync().then((existingUsers) => {
    if (!existingUsers || existingUsers.length === 0) {
      // If no users are found in Firebase or localStorage, use mock data
      debouncedSave(STORAGE_KEYS.USERS, MOCK_USERS)
      console.log(
        "[v0] Users initialized with mock data:",
        MOCK_USERS.map((u) => u.username),
      )
      // Sync mock users to Firebase if sync is enabled
      if (db && !firebaseSyncDisabled) {
        MOCK_USERS.forEach((user) => syncUserToFirebase(user))
      }
    }
  })
